...
const areAllFieldsFilled = computed(() => {
    return name.value.trim() && unit.value.trim() && quantity.value.toString().trim() && price.value.toString().trim();
})
...
<template>
    ...
    <div class="table desktop-tablet">
        ...
        <div class="grid-row" v-for="lineItem in lineRecorder.getLineRecordForProject(props.projectID)" :key="lineItem.itemID">
            ...
            <div class="grid-item data-display delete-button"><CloseIconButton @close="deleteLineItem(lineItem.itemID)"/></div>
        </div>
    </div>
    ...
    <div class="mobile">
        <div class="input-block" v-if="!project.completed">
            ...
            <div class="detail-input-containers">
                <label><input placeholder="Unit" class="table-input container" type="text" v-model="unit"></label>
                <label><input placeholder="Quantity" class="table-input container" type="text" v-model="quantity"></label>
                <label><input placeholder="Price" class="table-input container" type="text" v-model="price"></label>
            </div>
            <div><div class="container table-input total">€{{ total }}</div></div>
            <div><button @click="handleItemAddition" class="container submit-button" type="button" :disabled="!areAllFieldsFilled"><span class="mdi mdi-plus"></span></button></div>
        </div>

        <div class="data-container container" v-for="lineItem in lineRecorder.getLineRecordForProject(props.projectID)" :key="lineItem.itemID">
