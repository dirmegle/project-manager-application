  ...
  <main>
    ...
    <div class="container-no-content" v-if="activeClients.length === 0">
      ...
      <p class="no-content-message">
        You currently have no clients. <br />
        Add some!
      </p>
    </div>
    <div class="container-client-list">
      <v-expansion-panels>
        <v-expansion-panel
          elevation="0"
          class="container client-item"
          id="client-display-panel"
          ...
          <v-expansion-panel-text>
            ...
            <div class="button-wrapper">
              <BaseButton
                buttonStyle="filled"
                :disabled="false"
                @buttonClicked="showDeletionModal(client.clientID)"
                >Delete client</BaseButton
              >
            </div>
          </v-expansion-panel-text>
          <ConfirmationModal
            v-if="isDeletionModalVisible"
            title="Are you sure?"
            @cancel="isDeletionModalVisible = false"
            @confirm="handleClientDeletion"
            >You will no longer be able to select this client for new projects. If the client has
            any ongoing projects, they will remain until completed.
          </ConfirmationModal>
        </v-expansion-panel>
      </v-expansion-panels>
    </div>
